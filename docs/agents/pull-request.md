...
- **Product logic**: `src/main/`. A change here lands with its tests in
  `src/test/` in the same request.
- **Exempt**: `docs/`, `.github/`, `gradle/`, `*.md`, `*.kts`, build/tooling
  configuration, and dependency bumps with no behaviour change.
...
- **Default**: verify locally per `docs/agents/verification.md`, then open the
  request with the evidence.
- **When only a deployed environment can verify** — see the list in
  `docs/agents/verification.md` — open the request as a draft
  (`gh pr create --draft`), let the pipeline deploy, verify against it,
  attach evidence citing the pipeline or deployment id and the commit SHA,
  then mark it ready.
